...
#     return torch.stack((result, result, result, result), dim=1)
...
class RNNDetector(BaseDetector):
    ...
    def forward(self, x: torch.Tensor, prev_states: Optional[LstmStates] = None, token_mask: Optional[torch.Tensor] = None) \
        ...
        if False:
            output_dir = 'vis/img'
            img_rgb = x.clone()
            # img_rgb = torch.cat((img_rgb [0, :3, :, :].sum(0).unsqueeze(0), 0 * img_rgb [0, 12:15, :, :].sum(0).unsqueeze(0), img_rgb [0, 10:20, :, :].sum(0).unsqueeze(0)), dim=0)
            img_rgb = torch.cat((img_rgb [0, :10, :, :].sum(0).unsqueeze(0), 0 * img_rgb [0, 12:15, :, :].sum(0).unsqueeze(0), img_rgb [0, 10:20, :, :].sum(0).unsqueeze(0)), dim=0)
            img_array = (img_rgb.permute(1,2,0) * 255).cpu().numpy().astype(np.uint8)
            red_pixels = img_array[:, :, 0] > 0
            img_red = img_array[red_pixels]
            img_red[:, 2] = 0
            img_array[red_pixels] = img_red
        ...
        for stage_idx, stage in enumerate(self.stages):
            x, state, p, token = stage(x, prev_states[stage_idx], token_mask if stage_idx == 0 else None, r[:, stage_idx])
            tokens.append(token)
            states.append(state)
            stage_number = stage_idx + 1
            output[stage_number] = state[0]
            P.append(p)
        return output, states, P, img_array, tokens
...
class MaxVitAttentionPairCl(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos_emb: nn.Module, r: torch.Tensor, index_list) -> Tuple[torch.Tensor, ...]:
        ...
        return x, p_loss, r, [None, None], index_list
...
class RNNDetectorStage(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        for blk in self.att_blocks:
            x, p_loss, r, token, index_list = blk(x, self.pos_emb, r, index_list)
            P += p_loss
            tokens.append(token)
