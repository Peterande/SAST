...
class PartitionAttentionCl(nn.Module):
    ...
    def __init__(
        ...
        self.bounce_area = 1e-1
        self.act = nn.ReLU()
        self.drop = DropPath(drop_prob=0.05)
        # self.scale = nn.Linear(20, dim, bias=False)
    ...
    def get_indexes_window(self, scores: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        norm_window = (torch.norm(scores, dim=[2, 3], p=1.0) / temp).softmax(-1)
    ...
    def get_indexes_partition(self, scores: torch.Tensor, index_window: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        B, N, h, w = self.B, self.N, self.partition_size[0], self.partition_size[1]
        temp = self.dim / 32
        norm_partition = (torch.norm(scores, dim=[3], p=1.0) / temp).view(B * N, -1)[index_window].softmax(-1)
    ...
    def _partition_attn(self, x: torch.Tensor, pos_emb: nn.Module, r: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        scale = (r + 1e-6).mean(dim=1)[:, None, None, None]
        scores = self.act(self.to_scores(x.view(self.B, self.N, -1, self.dim)))
        # x = scores.view_as(x).sigmoid() * x # weighting x use sigmoid scores.
        x = (1 - scores.view_as(x).sigmoid()) * x # weighting x use sigmoid scores.

        with torch.no_grad():
            scores = 2e-3 / scale * scores
            scores[scores==torch.inf] = 0
        ...
        if len(index1):
            x = self.win_attn(x, index_window1, index1, blocked_index1, M1, K1)
        ...
        # #scores = scale * self.to_scores2(x.view(self.B, self.N, -1, self.dim)).float()
        # #x = (x + self.drop(scores.view_as(x).sigmoid() * x)) / 4
        # with torch.no_grad():
        #     index_window2 = self.get_indexes_window(100 * scores)
        #     index2, asy_index2, K2 = self.get_indexes_partition(10 * scores, index_window2)
        #     blocked_index2 = index2[torch.isin(index2, asy_index2, assume_unique=True, invert=True)]

        # M2 = len(index_window2)
        # if len(index2):
        #     x = self.grid_attn(x, index_window2, index2, blocked_index2, M2, K2)
        # x = grid_reverse(x, self.partition_size, (img_size[0], img_size[1]))

        # p_loss = torch.tensor((len(index1) + len(index2) - len(blocked_index1) - len(blocked_index2)) // self.B)
        p_loss = torch.tensor((len(index1) - len(blocked_index1)) // self.B) / (img_size[0] * img_size[1])
        # if True:
        #     temp = self.dim / 32
        #     norm_partition = (torch.norm(scores, dim=[3], p=1.0) / temp).view(self.N, -1)
        #     win = torch.zeros([self.N, 60, 3], device=x.device)
        ...
        #     win = jet_colors[norm_partition_scaled].squeeze()
        #     win = win[:, :, [2, 1, 0]]
        ...
        # if True:
        #     win = 255 * torch.ones([self.N, 60, 3], device=x.device)
        #     N = win.shape[0]
        ...
        #     temp = torch.tensor([70.0, 114.0, 196.0], device=win.device) # BGR
        ...
        #     return x, p_loss, r, [img_array1, img_array2]
        # else:
        return x, p_loss, r, None

    def forward(self, x: torch.Tensor, pos_emb: torch.Tensor, r: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        x, p_loss, r, img = self._partition_attn(x, pos_emb, r)
        return x, p_loss, r, img
