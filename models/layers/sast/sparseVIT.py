...
class PartitionAttentionCl(nn.Module):
    ...
    def __init__(
        ...
        self_attn_module = WindowSA_WindowPruningOnly
        ...
        # self.to_scores2 = nn.Linear(dim, dim)
    ...
    def get_indexes_window(self, scores: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        norm_window = (torch.norm(scores, dim=[2, 3], p=2) / temp).softmax(-1)
    ...
    def get_indexes_partition(self, scores: torch.Tensor, index_window: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        B, N, h, w = self.B, self.N, self.partition_size[0], self.partition_size[1]
        temp = self.dim / 32
        norm_partition = (torch.norm(scores, dim=[3], p=2) / temp).view(B * N, -1)[index_window].softmax(-1)
    ...
    def _partition_attn(self, x: torch.Tensor, pos_emb: nn.Module, r: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        # x = x + pos_emb(x)
        act_mag = torch.abs(x)
        act_mag = torch.mean(act_mag, -1, keepdim=True)
        x = window_partition(x, self.partition_size).view(self.B * self.N, -1, self.dim)
        win_act = window_partition(act_mag, self.partition_size).view(self.B, self.N, -1, 1)
        win_act = torch.linalg.vector_norm(win_act, ord=2, dim=2)
        res_nW = int(self.N * (1 - 0.5))
        _, w_index = torch.topk(win_act, res_nW, dim=1)
        w_index += torch.arange(self.B, device=x.device).view(-1, 1, 1) * self.N
        w_index = w_index.view(-1)
    
        x = self.win_attn(x, w_index, self.B * res_nW)
        ...
        win_act = grid_partition(act_mag, self.partition_size).view(self.B, self.N, -1, 1)
        win_act = torch.linalg.vector_norm(win_act, ord=2, dim=2)
        res_nW = int(self.N * (1 - 0.5))
        _, w_index = torch.topk(win_act, res_nW, dim=1)
        w_index += torch.arange(self.B, device=x.device).view(-1, 1, 1) * self.N
        ...
        x = self.grid_attn(x, w_index, self.B * res_nW)
        x = grid_reverse(x, self.partition_size, (img_size[0], img_size[1]))

        p_loss = torch.tensor((len(w_index) + len(w_index)) // self.B)
        return x, p_loss, r
