...
# @torch.no_grad()
# def measure_average_inference_time(model, dataset, batch_size, num_iters=100, warm_iters=10):
#     # note that warm-up iters. are excluded from the total iters.
#     iter_ = 0
#     t_total = 0
#     input_padder = InputPadderFromShape(desired_hw=(384, 640))
#     for i in tqdm.tqdm(range(10)):
#         for j in tqdm.tqdm(range(len(dataset.datapipe_list[i]))):
#             input = input_padder.pad_tensor_ev_repr(dataset.datapipe_list[i][j][DataType.EV_REPR][-1].cuda().unsqueeze(0)).repeat(batch_size, 1, 1, 1)
...
#                 t_total += t
#             iter_ += 1
#     return t_total / (iter_ - warm_iters)
...
# @torch.no_grad()
# def compute_fps(model, dataset, num_iters=300, warm_iters=5, batch_size=4, sparsity=0.0):
#     print(f"computing fps.. (num_iters={num_iters}, batch_size={batch_size}) "
#           f"warm_iters={warm_iters}, batch_size={batch_size}]")
#     assert num_iters > 0 and warm_iters >= 0 and batch_size > 0
#     model.cuda()
#     model.eval()
#     for batch_size in tqdm.tqdm(range(1, 1000)):
#         inputs = torch.rand((1, 20, 384, 640)).cuda().float()
#         inputs[:] = (inputs[:] > sparsity).float()
#         inputs = inputs.repeat(batch_size, 1, 1, 1)
#         t = measure_average_inference_time(model, inputs, 1, 0)
#     model.train()
#     print(f"FPS: {1.0 / t * batch_size}")  
#     return 1.0 / t * batch_size 
...
def compute_gflops(model, dataset, approximated=True, sparsity=0.0):
    ...
    if approximated:
        ...
        # for i in [0]:
            ...
            # kk = 20 if i == 2 else len(dataset.datapipe_list[i])
            for j in tqdm.tqdm(range(kk)):
                ...
                # r += non_zero_ratio(inputs) / len(dataset.datapipe_list[i])
            # print(f"non_zero_ratio_of_dataset[{i}]: {r}")
                with warnings.catch_warnings():
                    warnings.filterwarnings("ignore", category=RuntimeWarning)
