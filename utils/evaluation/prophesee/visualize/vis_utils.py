...
def draw_bboxes(img, boxes, labelmap=LABELMAP_GEN1, put_score=True) -> None:
    """
    draw bboxes in the image img
    """
    # colors = cv2.applyColorMap(np.arange(0, 255).astype(np.uint8), cv2.COLORMAP_HSV)
    colors = cv2.applyColorMap(np.arange(0, 179).astype(np.uint8), cv2.COLORMAP_HSV)
    colors = [tuple(*item) for item in colors.tolist()]
    ...
    for i in range(boxes.shape[0]):
        ...
        # if class_id == 1:
        #     color = (80, 176, 0)
        # color = colors[((class_id + 1) * 30) % 179]
        center = ((pt1[0] + pt2[0]) // 2, (pt1[1] + pt2[1]) // 2)
        cv2.rectangle(img, pt1, pt2, color, 2)
        # cv2.putText(img, class_name, (center[0], pt2[1] - 1), cv2.FONT_HERSHEY_SIMPLEX, 0.777, color)
        # if put_score:
        #     cv2.putText(img, f"{score:.2f}", (center[0], pt1[1] - 1), cv2.FONT_HERSHEY_SIMPLEX, 0.777, color)
